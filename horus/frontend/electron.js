...
app.whenReady().then(() => {
  // screen and ipcMain must be accessed after app is ready in Electron 36+
  ...
  overlayWin.webContents.openDevTools({ mode: "detach" }); // debug — remove later

  if (isDev) {
    overlayWin.loadURL("http://localhost:3000?overlay=true");
  } else {
    overlayWin.loadFile(path.join(__dirname, "build", "index.html"), {
      query: { overlay: "true" },
    });
  }
